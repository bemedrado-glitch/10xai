...
const GREETING: Message = {
  role: "assistant",
  content:
    "Hi, I'm Bernie  10XAI's customer support and sales concierge. I'm here to help. What's the workflow that's eating the most of your week right now?",
};
...
export function BernieChat() {
  ...
  // Listen for global "talk-to-bernie" events fired from Hero buttons
  useEffect(() => {
    const handler = () => {
      setOpen(true);
  ...
  async function send() {
    ...
    try {
      ...
      if (!res.ok || !res.body) {
        throw new Error(`Bernie is offline (${res.status})`);
      }
    ...
    <>
      ...
      {open && (
        ...
        >
          ...
          <header className="flex items-center justify-between border-b border-[var(--color-ink-300)] bg-[var(--color-ink)] px-4 py-3 text-[var(--color-cream)]">
            <div className="flex items-center gap-3">
              ...
              <div className="leading-tight">
                <p className="font-display text-sm font-bold">Bernie</p>
          ...
          <div
            ref={scrollRef}
            className="flex-1 overflow-y-auto bg-[var(--color-cream)] px-4 py-4"
          >
            <ul className="space-y-3">
              ...
              {error && (
                <li className="rounded-lg border border-[var(--color-danger)] bg-[var(--color-danger)]/5 px-4 py-2 text-xs text-[var(--color-danger)]">
                  {error}
                </li>
              )}
            </ul>
          ...
          >
            <input
              ...
              placeholder={
                limitReached
                  ? "Chat limit reached  book a call with Bernardo"
                  : "Ask Bernie anything..."
              }
          ...
          <p className="border-t border-[var(--color-ink-300)] bg-[var(--color-cream-100)] px-4 py-2 text-[10px] uppercase tracking-widest text-[var(--color-ink-500)]">
            Powered by Anthropic Claude  10XAI Concierge  {turnsRemaining}/{MAX_USER_TURNS} turns left
          </p>
        </div>
      )}
    </>
