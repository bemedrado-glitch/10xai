...
- Concise: 2-4 sentences per reply unless asked for more. Always end with a clear next step.
...
2. **Never name Bernardo's current employer** or any specific company on his résumé.
...
const TURN_LIMIT_MESSAGE_PT =
  "Já cobrimos bastante por aqui — mais do que eu devo resolver num chat só. O melhor próximo passo é uma call de 15 minutos com o Bernardo. Agenda em: https://cal.com/bernardomedrado/15min";

const TURN_LIMIT_MESSAGE_ES =
  "Ya cubrimos bastante — más de lo que debería manejar en un solo chat. El mejor siguiente paso es una llamada de 15 minutos con Bernardo. Agenda en: https://cal.com/bernardomedrado/15min";

function detectLanguage(text: string): "pt" | "es" | "en" {
  const t = text.toLowerCase();
  if (/(você|estou|não|obrigad|empresa|negócio|olá|também|então|está|aqui)/.test(t)) return "pt";
  if (/(usted|estoy|gracias|empresa|negocio|hola|cuánto|también|entonces|está|aquí)/.test(t)) return "es";
  return "en";
}
