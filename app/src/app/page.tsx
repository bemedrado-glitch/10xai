  ...
  return (
    <div className="flex flex-1 flex-col bg-[var(--color-cream)]">
      ...
      <header className="sticky top-0 z-30 w-full border-b border-[var(--color-ink-300)]/60 bg-[var(--color-cream)]/85 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 md:px-6 lg:px-8">
          ...
          <nav className="hidden items-center gap-8 text-sm font-medium md:flex">
            <Link href="/lighthouse" className="text-[var(--color-ink-700)] transition-colors hover:text-[var(--color-ink)]">Lighthouse</Link>
            <Link href="/engines" className="text-[var(--color-ink-700)] transition-colors hover:text-[var(--color-ink)]">Engines</Link>
            <Link href="/about" className="text-[var(--color-ink-700)] transition-colors hover:text-[var(--color-ink)]">About</Link>
      ...
      <section className="relative overflow-hidden">
        <div className="mx-auto max-w-6xl px-4 py-24 md:px-6 md:py-32 lg:px-8 lg:py-40">
          <div className="max-w-4xl">
            <p className="mb-6 text-xs font-medium uppercase tracking-[0.2em] text-[var(--color-ink-700)]">
              AI Operating Systems  US  Brasil  LatAm
            </p>
            <h1 className="font-display text-5xl font-black leading-[1.05] tracking-tight text-[var(--color-ink)] md:text-6xl lg:text-7xl">
              AI Operating Systems for the businesses that{" "}
              <span className="text-[var(--color-gold)]">can&rsquo;t afford</span> a 12-person team.
            </h1>
            <p className="mt-8 max-w-2xl text-lg leading-relaxed text-[var(--color-ink-700)] md:text-xl">
            ...
            <div className="mt-10 flex flex-col gap-4 sm:flex-row">
              ...
              <button
                type="button"
                className="inline-flex h-14 items-center justify-center rounded-lg border border-[var(--color-ink-300)] bg-[var(--color-cream-50)] px-8 text-base font-bold text-[var(--color-ink)] transition-colors hover:border-[var(--color-ink-700)] hover:bg-[var(--color-cream-100)]"
              >
                Talk to Dez
              </button>
            </div>
            <p className="mt-12 text-xs font-medium uppercase tracking-[0.2em] text-[var(--color-ink-500)]">
              ATD Master Trainer  AA-ISP Accredited  FGV  Top 100 Healthcare BR
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-[var(--color-cream)]">
        <div className="mx-auto max-w-6xl px-4 py-20 md:px-6 md:py-28 lg:px-8">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {[
              { name: "Lighthouse", tag: "Lead engine", desc: "We find local businesses with proven demand and ship them a website. Then we reach out.", anchor: "From $497 + $49/mo" },
            ...
            <article className="group flex flex-col rounded-xl border border-[var(--color-gold)]/30 bg-[var(--color-gold-100)]/40 p-6 transition-all hover:border-[var(--color-gold)] md:p-8">
              ...
              <h3 className="mt-3 font-display text-2xl font-bold text-[var(--color-ink)]">Dez</h3>
        ...
        <div className="mx-auto max-w-6xl px-4 py-12 md:px-6 lg:px-8">
          <div className="flex flex-col gap-8 md:flex-row md:items-center md:justify-between">
            ...
            <nav className="flex flex-wrap gap-x-8 gap-y-3 text-sm text-[var(--color-ink-700)]">
              <Link href="/lighthouse" className="hover:text-[var(--color-ink)]">Lighthouse</Link>
          ...
          <p className="mt-8 text-xs text-[var(--color-ink-500)]">
            &copy; 2026 10XAI  Apex / Raleigh, NC  contato@10xai.us
          </p>
        </div>
      </footer>
    </div>
