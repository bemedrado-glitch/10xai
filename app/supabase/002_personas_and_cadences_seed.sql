-- 10XAI Lighthouse — Personas & Cadences Seed
-- Run in Supabase SQL Editor against the "10xai" project (ytmanuajkffzoegkmqeb).
-- Idempotent: deletes any persona/cadence whose name is one of the seeded names, then re-inserts.
-- Source of truth: research/22-market-research-icp-and-cadences.md

-- ============================================================
-- 1. Clean slate for the seeded names (preserves your hand-made ones)
-- ============================================================
...
DELETE FROM cadences WHERE name IN (
  'Care Engine — Clinics (Booking Gap)',
  'Care Engine — Review Backlog',
  'Lighthouse — Local Service / Trades',
  'Lighthouse — Restaurants (Insta-only, BR-PT)',
...
-- ============================================================
-- 2. Personas — 10 ICPs aligned to 10XAI service offerings
-- ============================================================
-- filters JSON drives the Find Leads quick-apply: rating_min, min_reviews, no_website, category, country
-- Extra fields (service, tags, manual_qualify) are metadata for the operator, not auto-applied.

INSERT INTO personas (id, name, description, filters, active) VALUES
-- 1. Lighthouse-primary: tradespeople with no website
(gen_random_uuid(),
 'Local Service · No Website',
...
-- 2. Lighthouse-primary: BR restaurants using Instagram as their site
(gen_random_uuid(),
 'Restaurant · Insta-only (BR)',
...
-- 3. Lighthouse-primary: LatAm auto shops
(gen_random_uuid(),
 'Auto / Taller · No Website (LatAm)',
...
-- 4. Care Engine target: clinics WITH website but with booking pain
(gen_random_uuid(),
 'Clinic · Booking Bottleneck',
...
-- 5. Sales Engine target: pro services without a real CRM/sales motion
(gen_random_uuid(),
 'SMB · No CRM',
 'Professional services or B2B SMB with website but no real CRM/sales motion — leads tracked in spreadsheets or inbox. Sales Engine wedge. (Manual qualify: check site for absence of HubSpot/Salesforce/Pipedrive forms.)',
...
-- 6. Reach + Mind: pro firms with content gap
(gen_random_uuid(),
 'Pro Firm · Content Gap',
...
-- 7. Reach Engine: SMB missing social media
(gen_random_uuid(),
 'SMB · No Social Presence',
 'Has a website but no active Instagram/Facebook/TikTok presence (or last post 90+ days ago). Losing organic reach to competitors. Reach Engine wedge. (Manual qualify: search IG/FB/TikTok handles.)',
...
-- 8. Care Engine: review-rich, low response rate
(gen_random_uuid(),
 'SMB · Review Backlog',
...
-- 9. Care + Dez: D2C ecom support overload
(gen_random_uuid(),
 'D2C · Support Overload',
 'Shopify brands $500K–$10M revenue drowning in WISMO + size + return tickets. Real-time AI on PDP + Gorgias/Zendesk deflection. Care Engine + Dez wedge. (Sourced via LinkedIn/SimilarWeb, not Google Places.)',
...
-- 10. Bid Engine: pro firms with RFP throughput problem
(gen_random_uuid(),
 'Pro Firm · RFP Throughput',
...
-- ============================================================
-- 3. Cadences — one per persona, linked via persona_id
-- ============================================================
...
INSERT INTO cadences (id, name, description, persona_id, active)
SELECT gen_random_uuid(), c.cad_name, c.cad_desc, p.id, true
FROM (VALUES
...
-- ============================================================
-- 4. Cadence steps — email-only (cron is email-only in v1)
-- ============================================================
-- Variables interpolated by /api/cron/cadences:
--   {business_name}, {contact_name}, {city}, {phone}, {rating}
-- Bodies are plain text. HTML rendering can be added later.

-- Helper view for clarity
WITH cad AS (
...
-- SELECT count(*) AS personas_count FROM personas;                        -- expect 10+ (your hand-made + seeded)
-- SELECT count(*) AS cadences_count FROM cadences;                       -- expect 10+
-- SELECT count(*) AS steps_count FROM cadence_steps;                     -- expect ~50 (10 cadences × ~5 steps)
-- SELECT c.name, count(s.id) AS steps FROM cadences c LEFT JOIN cadence_steps s ON s.cadence_id = c.id GROUP BY c.name ORDER BY c.name;
